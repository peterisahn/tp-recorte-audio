...
double pd (const Instancia& instancia, int j, int t, vector<vector<double>>& memo, vector<vector<int>> &padre){
    ...
    for (int i = t-1; i< j; i++){
        ...
        double nuevo_res = minimo(res, pd(instancia, i, t-1, memo, padre)+instancia.costo(i,j));
        if (nuevo_res != res){
